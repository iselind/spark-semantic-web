...
class HybridFallbackStrategy(fallback: SparqlExecutionStrategy, parser: SparqlParser) extends SparqlExecutionStrategy {
  private val sparqlParser = new SparkOnlyStrategy(parser)

  if (fallback.isInstanceOf[SparkOnlyStrategy]) {
    throw new BadArgumentsException("Doesn't make sense to use SparkOnlyStrategy as fallback")
  }
